...
/* ********************** HOME ********************** */
    if(count($curpath)==1){
        $posts = $db->query('SELECT * FROM portfolio_post ORDER BY pub_date DESC');
...
/* ********************** VIEW ********************** */ 
    elseif($curpath[1] != 'manage' && $curpath[2] != 'delete'){
        $q = $db->prepare('SELECT * FROM portfolio_post WHERE `pub_date`=? AND `seotitle`=?');
...
/* ********************** MANAGE ********************** */ 
    elseif($curpath[1]=='manage'){
        ...
        if(!isset($safepost['savecontinue'])&&!isset($safepost['save'])&&isset($curpath[2])){
            $postQ = $db->prepare('SELECT * FROM portfolio_post WHERE `id`=?');
        ...
        elseif(isset($safepost['savecontinue'])||isset($safepost['save'])){
            ...
            {
                $postQ = $db->prepare('SELECT image FROM portfolio_post WHERE `id`=?');
                $postQ->execute(array($safepost['id']));
                $post = $postQ->fetch(PDO::FETCH_ASSOC);
                $dbQ = $db->prepare("DELETE FROM portfolio_post WHERE `id`=?");
                $dbQ->execute(array($safepost['id']));
                if(is_file($_SERVER['DOCUMENT_ROOT']."/".$pagedir."/media/".str_replace('portfolio/','',$post['image'])))
                {unlink($_SERVER['DOCUMENT_ROOT']."/".$pagedir."/media/".str_replace('portfolio/','',$post['image']));}
            ...
            else
            {
                ...
                $data['formerrors'] = validate_form($postform,$post,'portfolio_post');
                ...
                if(strlen($data['formerrors']) < 1){
                    ...
                    if(strlen($post['id']) > 0){
                        $binds[]=$post['id'];
                        $dbQ = $db->prepare("UPDATE portfolio_post SET ".$imgup."`title`=?,`seotitle`=?,`pub_date`=?,`body`=? WHERE `id`=?");
                        $dbQ->execute($binds);
                        $backto=$curpath[0].'/'.$dpath.$post['seotitle'];
                    }
                    else{
                        $dbQ = $db->prepare("INSERT INTO portfolio_post(".$imgadd."`title`,`seotitle`,`pub_date`,`body`) VALUES (?".str_repeat(',?',count($binds)-1).")");
                        $dbQ->execute($binds);
