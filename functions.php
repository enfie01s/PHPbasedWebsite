...
function datecompare($date1, $date2, $difflevel=1)
{
    ...
    if($difflevel > 1){
        ...
        if($diff->format('%y') > 0)
            { $return .= $y." Year".plural($y)." "; $count++; }
        if($diff->format('%m') > 0)
            { $return .= $m." Month".plural($m)." "; $count++; }
        if($diff->format('%d') > 0 && $count < $difflevel)
            { $return .= $d." Day".plural($d)." "; $count++; }
        if($diff->format('%h') > 0 && $count < $difflevel)
            { $return .= $h." Hour".plural($h)." "; $count++; }
        if($diff->format('%i') > 0 && $count < $difflevel)
            { $return .= $i." Minute".plural($i)." "; $count++; }
        if($diff->format('%s') > 0 && $count < $difflevel)
            { $return .= $s." Second".plural($s)." "; $count++; }
        return $return;
    }
    else
    {
...
if(!function_exists('password_verify') || !function_exists('password_hash'))
{
    function password_verify($matchtry, $matchto) {
        //if(crypt($matchtry,$matchto)==$matchto){ return true; }
        //else { return false; }
        return crypt($matchtry, $matchto) == $matchto;
    }
...
if(isset($safeget['makepass'])) {
    echo password_hash($_GET['makepass'], PASSWORD_BCRYPT);
}
...
if(isset($safepost['password'])) {
    ...
    elseif(isset($_COOKIE['loggedhash'])) { setcookie('loggedhash','',time()-2000); }  // Destroy cookie
    /* /handle posted form */
}
...
if(!isset($_SESSION['loggedin']) && isset($_COOKIE['loggedhash'])) {
    ...
    if($uQ->rowCount()>0) {
        ...
        unset($_SESSION['loggedin']); setcookie('loggedhash', '', time()-2000);
    }
}

if(isset($safepost['logout'])) {
    unset($_SESSION['loggedin']); setcookie("loggedhash", '', time()-2000, $cookie_path, $cookie_domain, 0);  // Destroy cookie
}
...
function validate_form($elements, $values, $table='')
{
    ...
    foreach($elements as $name => $data) {
        if($data['required'] == 'required' && strlen($values[$name]) < 1) {
            $return .= "<br>" . ucwords($name) . " is required.";
        }
    }
    return $return;
}
